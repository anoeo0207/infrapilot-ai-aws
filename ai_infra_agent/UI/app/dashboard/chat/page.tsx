...
import { AIResponse, ExecutionResult, WebSocketProgressMessage, Message, ExecutionPlanStep } from "@/types/data";
...
export default function ChatPage() {
    const [messages, setMessages] = useState<Message[]>([
        { id: "1", role: "assistant", content: "Hello! I'm InfraPilot AI, your infrastructure automation assistant. What would you like to automate today?", timestamp: new Date() }
    ]);
    ...
    const handleConfirmExecute = async () => {
        ...
        try {
            ...
            const ws = await executePlan(
                        ...
                        setAIResponse((prev) => {
                            ...
                            const newPlan = prev.executionPlan.map((step) =>
                                step.stepId === msg.stepId
                                    ? { ...step, status: msg.status || step.status, result: msg.result || step.result }
                                    : step
                            );
                ...
                (newStatus: string) => setStatusMessage(newStatus),
                async (result?: ExecutionResult) => {
                    setIsExecuting(false);
                    setIsExecutionComplete(true);
                    setExecutionFailed(false);

                    // === FIX: UPDATE ALL STEPS TO 'COMPLETED' ON SUCCESS ===
                    setAIResponse((prev) => {
                        if (!prev) return null;
                        const completedPlan = prev.executionPlan.map((step) => ({
                            ...step,
                            status: "completed" as "completed",
                        }));
                        return { ...prev, executionPlan: completedPlan };
                    });
                    // === END FIX ===
                    ...
                    if (finalResultToSave && aiResponse) {
                        try {
                            const userId = (session.user as any)?.id;
                            if (!userId) throw new Error("User ID not found in session.");
                            const descriptionAsJson = JSON.stringify(finalResultToSave, null, 2);
                            const actionType = (aiResponse as any).action || "generic_action";
                            setStatusMessage("Saving execution results to Supabase...");
                        ...
                        setStatusMessage("Execution finished, but backend did not send a result to save.");
                    }
                },
                (error: string) => {
    ...
    return (
        <DashboardLayout>
            ...
            <div className="space-y-6 mx-auto">
                ...
                <div className={`border rounded-lg p-4 text-sm font-medium flex items-center gap-2 ${executionFailed ? "bg-red-50 border-red-200 text-red-800" : isExecutionComplete ? "bg-green-50 border-green-200 text-green-800" : "bg-emerald-50 border-emerald-200 text-emerald-800"}`}>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6">
                        {messages.map((message) => (
                            <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                                <div className={`max-w-2xl ${message.role === "user" ? "bg-primary text-primary-foreground rounded-lg rounded-tr-none" : "bg-muted text-foreground rounded-lg rounded-tl-none"} p-4`}>
                    ...
                    <div className="border-t border-border p-6 bg-card">
                        <form onSubmit={handleSendMessage} className="flex gap-3 items-center">
                            <Input
                                ...
                                className="flex-1"
                            />
                            <Button type="submit" disabled={loading || isExecuting || !input.trim() || status !== 'authenticated'} className="gap-2">
                                            ...
                                            {step.details && Object.keys(step.details).length > 0 ? (
                                                <div className="p-3 bg-background rounded-md border text-sm font-mono space-y-1">
                                                    {Object.entries(step.details).map(([key, value]) => (
                                                        <div key={key}>
                                                            <span className="text-primary font-semibold">{key}:</span> <span>{JSON.stringify(value)}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            ) : (
                                                <p className="text-sm text-muted-foreground italic">No parameters for this step.</p>
                                            )}
                                        </div>
                                    )}
                                </div>
